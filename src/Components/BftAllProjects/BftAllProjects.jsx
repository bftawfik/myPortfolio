...
class BftAllProjects extends Component {
  ...
  componentWillReceiveProps(nProps) {
    let projects = !nProps.pageData ? [] : nProps.match.params.filter ? nProps.pageData.projects.filter(p => p.tags.filter(tag => tag.toLowerCase() === nProps.match.params.filter.toLowerCase()).length > 0) : nProps.pageData.projects;
  ...
  render(){
    ...
    return (
      <Row className="BftAllProjects">
        {projects.length===0 ? <Col xl="12"><BftPreloader/></Col> : null}
        ...
        {projects.map((projData, ndx) => <Col lg="6" md="12" key={ndx}><BftProject data={projData} resetProjectsState={this.resetProjectsState.bind(this, ndx)} isActive={this.state.projectsState[ndx].isActive}/></Col> )}
  ...
  resetProjectsState(key){
    let newProjectsState = this.state.projectsState.map((p, ndx) => ({isActive: key == ndx ? true : false}));
